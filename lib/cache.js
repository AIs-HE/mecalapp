        ...
        for (let i = 0; i < q.length; i++) {
            ...
            try {
                if (op.type === 'create_memory') {
                    // include sensible defaults so server/database constraints are satisfied
                    const res = await fetch('/api/project_memories', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ project_id: op.payload.project_id, memory_type: op.payload.memory_type, version: op.payload.version || '1.0', status: op.payload.status || 'draft' }) })
...
// Draft helpers (per-memory draft caching)
export function getDraftKey(memoryId) {
    return `mecalapp_draft_${memoryId}`
}

export function getDraft(memoryId) {
    if (!memoryId) return null
    return readJSON(getDraftKey(memoryId)) || null
}

export function setDraft(memoryId, draftArray) {
    if (!memoryId) return null
    try {
        const now = new Date().toISOString()
        const existing = getDraft(memoryId) || { draftArray: null, draftLastModified: null, lastSavedToDbAt: null }
        const entry = { draftArray, draftLastModified: now, lastSavedToDbAt: existing.lastSavedToDbAt || null }
        writeJSON(getDraftKey(memoryId), entry)
        return entry
    } catch (e) {
...
export function updateLastSavedToDb(memoryId, dbLastModified) {
    if (!memoryId) return null
    try {
        const existing = getDraft(memoryId) || { draftArray: null, draftLastModified: null, lastSavedToDbAt: null }
        existing.lastSavedToDbAt = dbLastModified
        writeJSON(getDraftKey(memoryId), existing)
        return existing
    } catch (e) {
        console.error('updateLastSavedToDb error', e)
        return null
    }
}

export function removeDraft(memoryId) {
    if (!memoryId) return
    try {
        localStorage.removeItem(getDraftKey(memoryId))
    } catch (e) {
