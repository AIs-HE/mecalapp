...
const MemoryCard = dynamic(() => import('../components/MemoryCard'), { ssr: false })
const BackgroundRects = dynamic(() => import('../components/BackgroundRects'), { ssr: false })
const AddProjectCard = dynamic(() => import('../components/AddProjectCard'), { ssr: false })
const AddMemoryCard = dynamic(() => import('../components/AddMemoryCard'), { ssr: false })
...
export default function Projects() {
    ...
    useEffect(() => {
        let mounted = true
        // load session and watch auth state; do not perform a route replace here so
        // `/projects` remains the canonical route for the projects UI.
        supabase.auth.getSession().then(({ data }) => {
            if (!mounted) return
            setUser(data?.session?.user ?? null)
            // if an initial session exists, try to load profile role and persist to localStorage
    ...
    return (
        <div className="font-sans min-h-screen relative flex flex-col">
            ...
            <main className="app-main-fixed">
                <div className="p-5 max-w-[1400px] mx-auto w-full box-border">
                    {selectedProject ? (
