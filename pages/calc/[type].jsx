...
export default function CalcTypePage() {
    ...
    useEffect(() => {
        ...
        ;(async () => {
            ...
            try {
                ...
                const memoryId = q.memory_id || q.memoryId || null
                const projectId = q.project_id || q.projectId || null
                ...
                // load profile (role and full_name) if logged in
                let profile = null
                if (user) {
                    try {
                ...
                // if we don't have memory but have projectId, try to find a memory of this type in the project
                if (!memory && projectId) {
                    try {
                        const res = await fetch(`/api/project_memories?project_id=${encodeURIComponent(projectId)}`)
                        const j = await res.json()
                        if (res.ok && Array.isArray(j.memories)) {
                            const found = j.memories.find(m => (m.id === memoryId) || (String((m.type||m.memory_type||'')).toLowerCase() === String(type).toLowerCase()))
                            if (found) memory = found
                        }
                    } catch (e) { /* ignore */ }
                }

                // load project info if projectId present or from memory
                let project = null
